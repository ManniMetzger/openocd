...
static int aji_client_build_buffers(
	...
	*read_buffer = NULL;
	req = 0;
	for (unsigned int i = 0; i < cmd->num_tap_fields; i++) {
...
static AJI_ERROR aji_client_relock_tap(struct jtag_tap *tap)
{
	jtagservice_unlock();
	return jtagservice_lock(tap);
}
...
static int aji_client_scan(struct scan_command *const cmd)
{
	...
	int retval = aji_client_build_buffers(cmd, &bit_count, &write_buffer, &read_buffer);
	if (retval != ERROR_OK)
		return retval;
	...
	if (cmd->ir_scan) {
		if (cmd->tap_is_sld) {
			if (!write_buffer || bit_count > 32) {
				LOG_ERROR("AJI SLD overlay instructions must contain 1..32 bits");
				status = AJI_INVALID_PARAMETER;
			} else {
				DWORD instruction = 0;
				for (DWORD i = 0; i < DIV_ROUND_UP(bit_count, 8); i++)
					instruction |= (DWORD)write_buffer[i] << (i * 8);
				DWORD capture = 0;
				...
				if (status == AJI_NO_ERROR && read_buffer) {
					for (DWORD i = 0; i < DIV_ROUND_UP(bit_count, 8); i++)
						read_buffer[i] = (BYTE)(capture >> (i * 8));
				}
			}
		} else {
			status = c_aji_access_ir_a(open_id, bit_count, write_buffer, read_buffer, 0);
		}
	} else {
	...
	if (read_buffer && aji_client_read_buffer(read_buffer, cmd) != ERROR_OK) {
		free(write_buffer);
		free(read_buffer);
		return ERROR_FAIL;
	}
...
static int aji_client_execute_queue(struct jtag_command *queue)
{
	...
	for (cmd = queue; ret == ERROR_OK && cmd != NULL;
		 cmd = cmd->next) {

		keep_alive();

		find_next_active_tap(cmd, &tap);
		AJI_ERROR lock_status = jtagservice_lock(tap);
		if (lock_status == AJI_CHAIN_IN_USE) {
			LOG_WARNING("AJI chain in use while locking TAP; "
				"releasing stale lock and retrying once");
			lock_status = aji_client_relock_tap(tap);
		}
		if (lock_status != AJI_NO_ERROR && lock_status != AJI_LOCKED) {
			LOG_ERROR("Cannot lock AJI TAP: %d (%s)", lock_status,
				c_aji_error_decode(lock_status));
			ret = ERROR_FAIL;
			break;
		}
		...
		case JTAG_RESET:
			...
			break;
		case JTAG_RUNTEST:
			ret = aji_client_runtest(
				cmd->cmd.runtest->num_cycles,
				cmd->cmd.runtest->end_state
			);
			break;
		case JTAG_STABLECLOCKS:
			ret = aji_client_runtest(cmd->cmd.stableclocks->num_cycles, TAP_IDLE);
			break;
		case JTAG_TLR_RESET:
			ret = aji_client_goto_tlr();
			break;
		case JTAG_PATHMOVE:
			LOG_ERROR(
...
static const struct command_registration vjtag_subcommand_handlers[] = {
	{
		.name = "create",
		.mode = COMMAND_CONFIG,
		.handler = handle_aji_vjtag_create,
		.help = "Create a new virtual TAP instance on tapname, "
			"and appends it to the scan chain.",
		.usage = "name '-chain-position' tapname '-expected-id' idcode "
			"['-ignore-version'] ",
	},
