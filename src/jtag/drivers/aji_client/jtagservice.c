...
AJI_ERROR jtagservice_lock(const struct jtag_tap* const tap)
{
	if(tap && jtag_tap_on_all_vtaps_list(tap)) {
		...
		AJI_ERROR status = jtagservice_device_index_by_idcode(tap->parent->idcode,
			&tap_index);
	...
	if(tap) {
		DWORD tap_index = UINT32_MAX;
		AJI_ERROR status = jtagservice_device_index_by_idcode(
			tap->idcode,
			&tap_index
		);
		if (status != AJI_NO_ERROR)
	...
	const char* name, uint32_t idcode)
{
	log_printf_lf(level, __FILE__, __LINE__, __func__,
		...
		jep106_table_manufacturer(EXTRACT_JEP106_BANK(idcode), EXTRACT_JEP106_ID(idcode)),
 ...
 */
static bool vjtag_examine_chain_match_tap(struct jtag_tap* vtap) {
	AJI_ERROR status = AJI_NO_ERROR;
...
int jtagservice_jtag_examine_chain(void)
{
	...
	for (t = 0, tap = jtag_tap_next_enabled(NULL);
		...
		if (tap == NULL) {
			...
			LOG_WARNING("AUTO %s - use \"jtag newtap %s %s -irlen %u "
				"-expected-id 0x%08" PRIx32 "\"", tap->dotted_name,
				tap->chip, tap->tapname, tap->ir_length, tap->idcode);
		}
	...
	for (struct jtag_tap* vtap = vjtag_all_taps();
		vtap != NULL;
		vtap = (struct jtag_tap*)vtap->next_tap) {
		vtap->idcode = vtap->expected_ids[0];
		vtap->has_idcode = true;

		if (!vjtag_examine_chain_match_tap(vtap)) {
			vtap->has_idcode = false;
			vtap->idcode = 0;
			retval = ERROR_JTAG_INIT_SOFT_FAIL;
			continue;
		}
