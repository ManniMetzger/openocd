...
#define FNAME_AJI_GET_NODES_BI__MINGW64 "_Z13aji_get_nodesP9AJI_CHAINmP11AJI_HIER_IDPmP12AJI_HUB_INFOi"
AJI_ERROR AJI_API c_aji_get_nodes_bi(
    ...
    assert(c_jtag_client_lib != NULL);
    typedef AJI_ERROR(*ProdFn)(AJI_CHAIN_ID, DWORD, AJI_HIER_ID*, DWORD*, AJI_HUB_INFO*);
    ProdFn pfn = (ProdFn)(void*)GetProcAddress(c_jtag_client_lib, FNAME_AJI_GET_NODES_BI__MINGW64);
    assert(pfn != NULL);
    if (pfn == NULL) {
        return AJI_FAILURE;
    }
...
AJI_ERROR c_aji_open_device_a(AJI_CHAIN_ID chain_id, DWORD tap_position, AJI_OPEN_ID* open_id, const AJI_CLAIM2* claims, DWORD claim_n, const char* application_name) {
    ...
    ProdFn pfn = (ProdFn)(void*)GetProcAddress(c_jtag_client_lib, FNAME_AJI_OPEN_DEVICE_A__MINGW64);
    assert(pfn != NULL);
    if (pfn == NULL) {
        return AJI_FAILURE;
    }
...
AJI_ERROR c_aji_access_dr_a(AJI_OPEN_ID open_id, DWORD length_dr, DWORD flags, DWORD write_offset, DWORD write_length, const BYTE* write_bits, DWORD read_offset, DWORD read_length, BYTE* read_bits, DWORD batch) {
    ...
    ProdFn pfn = (ProdFn)(void*)GetProcAddress(c_jtag_client_lib, FNAME_AJI_ACCESS_DR_A__MINGW64);
    assert(pfn != NULL);
    if (pfn == NULL) {
        return AJI_FAILURE;
    }
